...
$imagelist = array();
foreach ($graphlist_keys as $key) {
  $imageid = username2id($dbh, $key);
  $sql = "select * from avater.image where id = :id";
  $stmt = $dbh->prepare($sql);
...
/*
  ...
  foreach($image as $key){
    array_push($imagelist_key, $key['raw_data']);
  }
  var_dump($imagelist_key);
*/
  //var_dump($image['raw_data']);      src="data:%s;base64,%s"
  array_push($imagelist, $image['id']!=null ? "data:".image_type_to_mime_type($image['type']).";base64,".base64_encode($image['thumb_data']):null);
}
//var_dump($imagelist);
//exit();
$enc_imagelist = json_encode($imagelist);
...
<body>
<div id="header">
	<h1>Map<a href="login/logout.php"><i class="fa fa-sign-out"></i></a></h1>
</div>
...
<script>
  ...
  var imagelist = <?php echo($enc_imagelist); ?>;
  ...
  for(var i = 0; i < namelist.length; i++){
    ...
    nodes[i].avater = imagelist[i] != null ? imagelist[i] : "image/noimage.png";//"image/chrome.png";
  ...
  for(var i = 0; i < namelist.length; i++){
    ...
    links[i].distance = ((Math.random()*4)+2)*50;
  }
...
text = node.append("text")
      .text(function(d, i){return "Friends:" + d.countFriend.toString() + "/" + d.countPerson.toString();})
      .attr({
        'text-anchor': "middle",
        'dy': ".65em",
        'fill': "black",
